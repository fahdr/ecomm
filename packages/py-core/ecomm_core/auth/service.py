...
For Developers:
    ...
    All functions accept a `settings` parameter to avoid global state.
...
def create_access_token(user_id: uuid.UUID, *, secret_key: str, algorithm: str = "HS256", expire_minutes: int = 15) -> str:
    """
...
def create_refresh_token(user_id: uuid.UUID, *, secret_key: str, algorithm: str = "HS256", expire_days: int = 7) -> str:
    """
...
def decode_token(token: str, *, secret_key: str, algorithm: str = "HS256") -> dict | None:
    """
...
async def provision_user(
    ...
    *,
    ...
    Args:
        ...
        service_name: Current service name (for API key prefix).
    ...
    raw_key = f"{service_name[:2]}_live_{secrets.token_urlsafe(32)}"
