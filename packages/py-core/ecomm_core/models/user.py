"""
User model shared across all ecomm SaaS services.

Each service maintains its own independent user table. Users can sign up
directly or be provisioned via the dropshipping platform API.

For Developers:
    The `plan` field is denormalized from the Subscription model for fast
    lookups. `external_platform_id` and `external_store_id` are set when
    a user is provisioned from the dropshipping platform.
...
class PlanTier(str, enum.Enum):
    """
    Subscription plan tiers available for services.
...
class User(Base):
    """
    User account for an ecomm SaaS service.
    ...
    Attributes:
        ...
        external_platform_id: Dropshipping platform user ID (if provisioned).
        external_store_id: Dropshipping platform store ID (if linked).
