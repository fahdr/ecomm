"""
Test fixtures for the SourcePilot backend.

Uses a dedicated PostgreSQL schema (``sourcepilot_test``) to isolate from
...
def _make_engine():
    ...
    def set_search_path(dbapi_conn, connection_record):
        ...
        operations target the ``sourcepilot_test`` schema instead of
        ``public``.
        """
...
async def create_tables():
    """
    Create the ``sourcepilot_test`` schema and all tables once at the
        ...
        try:
            ...
            await term_conn.execute(
                ...
                WHERE datname = current_database()
                  ...
                  AND query LIKE '%sourcepilot_test%'
                """
            )
    ...
    # Step 3: Create tables with a fresh engine (search_path = sourcepilot_test only)
    last_error = None
    for attempt in range(3):
        engine = _make_engine()
        try:
...
async def truncate_tables():
    ...
    sourcepilot_test schema. Retries up to 3 times on transient
    connection errors.
    """
