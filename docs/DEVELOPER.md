...
│   │   └── ... (16 more: subscription, discount, category, review, etc.)
...
Themes generate CSS variables via `storefront/src/lib/theme-utils.ts`:
...
The storefront uses `motion` (framer-motion) for animations via reusable primitives in `storefront/src/components/motion-primitives.tsx`:
