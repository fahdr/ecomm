"""
Platform webhook endpoints for PostPilot.
...
For Project Managers:
    Platform events enable automated social media post creation when
    products are listed on the dropshipping platform. PostPilot handles
    ``product.created`` events to queue social media posts across
    connected channels (Instagram, Twitter/X, Facebook, TikTok).

For End Users:
    When you add a new product on the main platform, PostPilot
    automatically drafts and queues social media posts to promote it.
"""
...
async def platform_event_webhook(request: Request):
    ...
    PostPilot handles:
        - ``product.created`` -- queues social media posts across connected
          channels to promote the newly listed product.
    ...
    # Route to PostPilot-specific handlers
    if event_type == "product.created":
        logger.info(
            "Social media post queued for new product: %s",
            event_data.get("product_id", "unknown"),
        )
        actions.append("social_post_queued")
