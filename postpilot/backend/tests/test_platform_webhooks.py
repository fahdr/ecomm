"""
Platform webhook endpoint tests for PostPilot.
...
For QA Engineers:
    ...
    - Verifies correct event routing:
        product.created -> social_post_queued
...
async def test_platform_webhook_product_created(client: AsyncClient):
    """POST /webhooks/platform-events with product.created returns social_post_queued."""
    payload = json.dumps({
    ...
    assert "social_post_queued" in data["actions"]
