"""
Test fixtures for the PostPilot backend.

Uses shared test utilities from ecomm_core for database setup and auth helpers.

For Developers:
    Uses a dedicated PostgreSQL database (``postpilot_test``) to isolate from
...
def _make_engine():
    ...
    Returns:
        AsyncEngine: A new async engine using NullPool for test isolation.
    """
...
async def _ensure_test_db():
    """
    Create the postpilot_test database if it does not exist.
...
async def create_tables():
    ...
    Step 2: Terminate stale connections from prior test runs.
    Step 3: Drop all tables and recreate them with a fresh engine.
    """
    ...
    try:
        async with engine.begin() as conn:
            ...
            # Drop enum types that may linger from previous runs
            await conn.execute(text("DROP TYPE IF EXISTS poststatus CASCADE"))
            await conn.execute(text("DROP TYPE IF EXISTS socialplatform CASCADE"))
            await conn.execute(text("DROP TYPE IF EXISTS queuestatus CASCADE"))
            await conn.execute(text("DROP TYPE IF EXISTS plantier CASCADE"))
...
async def truncate_tables():
    ...
    Ensures each test starts with a clean database state without
    the overhead of dropping and recreating tables.
    """
