"""
TrendScout — FastAPI Application Entry Point.

Uses shared router factories from ecomm_core for auth, billing, health,
API keys, usage, and webhooks. Service-specific routers are included
separately.

For Developers:
    Add new routers in the 'Service-specific routers' section.
    Shared routers are created via factory functions from ecomm_core.

For QA Engineers:
    The app starts with ``uvicorn app.main:app --reload``.
    API docs at /docs (Swagger UI) and /redoc.

For End Users:
    Access the API at http://localhost:8101/api/v1/
    Interactive docs at http://localhost:8101/docs
"""
...
app = FastAPI(
    ...
    description=f"REST API for {settings.service_display_name} — AI-Powered Product Research",
    version="1.0.0",
    lifespan=lifespan,
)
...
# Shared routers from ecomm_core
app.include_router(
    create_health_router(settings.service_name), prefix="/api/v1"
)
app.include_router(
    create_auth_router(get_db, get_current_user, get_current_user_or_api_key),
    prefix="/api/v1",
)
app.include_router(
    create_billing_router(get_db, get_current_user, PLAN_LIMITS),
    prefix="/api/v1",
)
app.include_router(
    create_webhook_router(async_session_factory, PLAN_LIMITS),
    prefix="/api/v1",
)
app.include_router(
    create_api_keys_router(get_db, get_current_user),
    prefix="/api/v1",
)
app.include_router(
    create_usage_router(
        get_db, get_current_user_or_api_key,
        lambda db, user: get_usage(db, user, PLAN_LIMITS),
    ),
    prefix="/api/v1",
)
