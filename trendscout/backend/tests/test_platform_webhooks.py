"""
Platform webhook endpoint tests for TrendScout.
...
For QA Engineers:
    ...
    - Verifies correct event routing: product.created -> product_tracked.
...
async def test_platform_webhook_product_created(client: AsyncClient):
    """POST /webhooks/platform-events with product.created returns product_tracked."""
    payload = json.dumps({
    ...
    assert "product_tracked" in data["actions"]
