"""
Platform webhook endpoint tests for ShopChat.
...
For QA Engineers:
    ...
    - Verifies correct event routing:
        product.created -> knowledge_base_updated
        product.updated -> knowledge_base_updated
...
async def test_platform_webhook_product_created(client: AsyncClient):
    """POST /webhooks/platform-events with product.created returns knowledge_base_updated."""
    payload = json.dumps({
    ...
    assert "knowledge_base_updated" in data["actions"]


@pytest.mark.asyncio
async def test_platform_webhook_product_updated(client: AsyncClient):
    """POST /webhooks/platform-events with product.updated returns knowledge_base_updated."""
    payload = json.dumps({
        "event": "product.updated",
        "data": {"product_id": "prod_123", "title": "Updated Gadget"},
    }).encode()
    resp = await client.post(
        "/api/v1/webhooks/platform-events",
        content=payload,
