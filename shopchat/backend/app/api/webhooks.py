"""
Platform webhook endpoints for ShopChat.
...
For Project Managers:
    Platform events enable automated knowledge base updates when products
    are created or modified on the dropshipping platform. ShopChat handles
    ``product.created`` and ``product.updated`` events to keep the AI
    chatbot's product knowledge current and accurate.

For End Users:
    When you add or update products on the main platform, ShopChat's AI
    assistant automatically learns about those changes so it can answer
...
async def platform_event_webhook(request: Request):
    ...
    ShopChat handles:
        - ``product.created`` -- updates the AI knowledge base with
          information about the newly listed product so the chatbot can
          answer customer questions about it.
        - ``product.updated`` -- refreshes the AI knowledge base to
          reflect product changes (pricing, descriptions, availability).
    ...
    # Route to ShopChat-specific handlers
    if event_type == "product.created":
        logger.info(
            "Knowledge base updated for new product: %s",
            event_data.get("product_id", "unknown"),
        )
        actions.append("knowledge_base_updated")
    elif event_type == "product.updated":
        logger.info(
            "Knowledge base updated for modified product: %s",
            event_data.get("product_id", "unknown"),
        )
        actions.append("knowledge_base_updated")
