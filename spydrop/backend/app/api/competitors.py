...
from app.services.analysis_service import analyze_competitor
...
async def analyze_competitor_endpoint(
    ...
    """
    Generate an AI-powered competitive analysis for a competitor.

    Uses the centralized LLM Gateway to analyze the competitor's product
    catalog, pricing patterns, and market positioning. Returns structured
    insights including competitive advantages, market gaps, and pricing
    ...
    Returns:
        Dict with structured competitive analysis results.

    Raises:
        HTTPException 400: If the competitor ID is not a valid UUID.
        HTTPException 404: If the competitor is not found or not owned by user.
    """
    import uuid as uuid_mod

    try:
        cid = uuid_mod.UUID(competitor_id)
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid competitor ID")

    try:
        result = await analyze_competitor(db, current_user.id, cid)
        return result
    except ValueError as e:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
