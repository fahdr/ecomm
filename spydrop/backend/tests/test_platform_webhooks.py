"""
Platform webhook endpoint tests for SpyDrop.
...
For QA Engineers:
    ...
    - Verifies correct event routing:
        order.created -> competitor_price_check_queued
...
async def test_platform_webhook_order_created(client: AsyncClient):
    """POST /webhooks/platform-events with order.created returns competitor_price_check_queued."""
    payload = json.dumps({
        "event": "order.created",
        "data": {"order_id": "ord_123", "products": ["prod_1", "prod_2"]},
    }).encode()
    ...
    assert "competitor_price_check_queued" in data["actions"]
