"""
Platform webhook endpoint tests for RankPilot.
...
For QA Engineers:
    ...
    - Verifies correct event routing:
        product.created -> seo_schema_created
        product.updated -> seo_schema_updated
...
async def test_platform_webhook_product_created(client: AsyncClient):
    """POST /webhooks/platform-events with product.created returns seo_schema_created."""
    payload = json.dumps({
        "event": "product.created",
        "data": {"product_id": "prod_123", "title": "Test Product"},
    }).encode()
    ...
    assert "seo_schema_created" in data["actions"]


@pytest.mark.asyncio
async def test_platform_webhook_product_updated(client: AsyncClient):
    """POST /webhooks/platform-events with product.updated returns seo_schema_updated."""
    payload = json.dumps({
        "event": "product.updated",
        "data": {"product_id": "prod_123", "title": "Updated Product"},
    }).encode()
    resp = await client.post(
        "/api/v1/webhooks/platform-events",
        content=payload,
    ...
    data = resp.json()
    assert data["status"] == "ok"
    assert "seo_schema_updated" in data["actions"]
