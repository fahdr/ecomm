...
For Developers:
    Uses a dedicated ``rankpilot_test`` PostgreSQL database to isolate from
    other services sharing the main database. Tables are created once at session
    start and truncated before each test. NullPool ensures no connection reuse.
...
from ecomm_core.testing import register_and_login  # noqa: F401
...
# Import all model classes so Base.metadata is fully populated
...
async def _ensure_test_db():
    """
    Create the rankpilot_test database if it does not exist.
