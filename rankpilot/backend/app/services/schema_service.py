...
# ── Standalone Schema.org Generators ─────────────────────────────────────


def generate_product_schema(product_data: dict) -> dict:
    """
    Generate a JSON-LD Product schema from product data.

    Creates a valid schema.org Product structured data object
    suitable for embedding in an HTML page as JSON-LD.

    Args:
        product_data: Dict with product fields. Expected keys:
            - name (str): Product name (required).
            - description (str): Product description.
            - image (str): Product image URL.
            - price (str|float): Product price.
            - currency (str): Price currency code (default 'USD').
            - availability (str): Stock status ('InStock', 'OutOfStock', etc.).
            - url (str): Product page URL.
            - brand (str): Brand name.
            - sku (str): Product SKU.
            - rating_value (float): Average rating (optional).
            - review_count (int): Number of reviews (optional).

    Returns:
        A valid JSON-LD Product schema dict with @context and @type.
    """
    schema: dict = {
        "@context": "https://schema.org",
        "@type": "Product",
    ...
    if product_data.get("brand"):
        schema["brand"] = {"@type": "Brand", "name": product_data["brand"]}

    # Offer
    offer: dict = {"@type": "Offer"}
    if product_data.get("price") is not None:
        offer["price"] = str(product_data["price"])
    offer["priceCurrency"] = product_data.get("currency", "USD")
    ...
    if product_data.get("rating_value") is not None and product_data.get("review_count") is not None:
        schema["aggregateRating"] = {
            "@type": "AggregateRating",
            "ratingValue": str(product_data["rating_value"]),
            "reviewCount": str(product_data["review_count"]),
        }
...
def generate_faq_schema(faqs: list[dict]) -> dict:
    ...
    Creates a valid schema.org FAQPage structured data object
    for enabling FAQ rich snippets in search results.

    Args:
        faqs: List of dicts, each with 'question' and 'answer' keys.
            Example: [{"question": "What is SEO?", "answer": "SEO stands for..."}]

    Returns:
        A valid JSON-LD FAQPage schema dict with @context and @type.
    """
    main_entity = []
    for faq in faqs:
...
def generate_breadcrumb_schema(breadcrumbs: list[dict]) -> dict:
    """
    Generate a JSON-LD BreadcrumbList schema from breadcrumb items.

    Creates a valid schema.org BreadcrumbList structured data object
    for displaying breadcrumb navigation in search results.

    Args:
        breadcrumbs: List of dicts with 'name' and 'url' keys, in order.
            Example: [{"name": "Home", "url": "https://example.com/"},
                       {"name": "Products", "url": "https://example.com/products"}]

    Returns:
        A valid JSON-LD BreadcrumbList schema dict with @context and @type.
    """
    items = []
...
def generate_aggregate_rating_schema(rating: float, count: int) -> dict:
    """
    Generate a JSON-LD AggregateRating schema.

    Creates a standalone AggregateRating structured data object.
    Typically embedded within a Product or LocalBusiness schema,
    but can be used independently.

    Args:
        rating: The average rating value (e.g., 4.5).
        count: The total number of ratings/reviews.

    Returns:
        A valid JSON-LD AggregateRating schema dict with @context and @type.
    """
    return {
        "@context": "https://schema.org",
        "@type": "AggregateRating",
