...
class StoreConnectionCreate(BaseModel):
    ...
    Attributes:
        platform: E-commerce platform ('shopify', 'woocommerce', 'bigcommerce', 'custom').
        store_url: The store's base URL.
        api_key: Optional API key for store access.
        api_secret: Optional API secret for store access.
    """

    platform: str = Field(
        ...,
        pattern="^(shopify|woocommerce|bigcommerce|custom)$",
        description="E-commerce platform",
    )
    store_url: str = Field(..., min_length=5, max_length=512, description="Store base URL")
    api_key: str | None = Field(None, description="API key (will be encrypted)")
    api_secret: str | None = Field(None, description="API secret (will be encrypted)")
...
class StoreConnectionUpdate(BaseModel):
    ...
    store_url: str | None = Field(None, min_length=5, max_length=512)
...
class StoreConnectionResponse(BaseModel):
    ...
    Note: Encrypted fields are not returned in the response.
...
class SitemapUrlEntry(BaseModel):
    """
    A single URL entry for sitemap generation.

    Attributes:
        loc: The page URL (required).
        lastmod: Last modification date (ISO 8601, optional).
        changefreq: Change frequency (optional).
        priority: Priority from 0.0 to 1.0 (optional).
    """

    loc: str = Field(..., description="Page URL")
    lastmod: str | None = Field(None, description="Last modification date")
    changefreq: str | None = Field(
        None,
        pattern="^(always|hourly|daily|weekly|monthly|yearly|never)$",
        description="Change frequency",
    )
    priority: float | None = Field(None, ge=0.0, le=1.0, description="Priority (0.0-1.0)")


class SitemapGenerateRequest(BaseModel):
    """
    Request schema for generating an XML sitemap.

    Attributes:
        urls: List of URL entries to include in the sitemap.
    """

    urls: list[SitemapUrlEntry] = Field(..., description="URLs to include in sitemap")
...
class PageAnalyzeRequest(BaseModel):
    """
    Request schema for analyzing a single page.

    Attributes:
        url: The full URL of the page to analyze.
    """

    url: str = Field(..., min_length=10, description="Full URL to analyze (https://...)")


class PageAnalysisResponse(BaseModel):
    """
    Response schema for a page SEO analysis.

    Attributes:
        url: The analyzed page URL.
        title: Page title tag content.
        meta_description: Meta description content.
        h1_tags: List of H1 heading texts.
        h2_tags: List of H2 heading texts.
        h3_tags: List of H3 heading texts.
        images_without_alt: Count of images missing alt text.
        internal_link_count: Number of internal links found.
        external_link_count: Number of external links found.
        word_count: Approximate page word count.
        canonical_url: Canonical URL (if specified).
        og_tags: Open Graph tags found.
        meta_score: Meta tag quality score (0-100).
        content_score: Content quality score (0-100).
        technical_score: Technical SEO score (0-100).
        mobile_score: Mobile-friendliness score (0-100).
        performance_score: Performance score (0-100).
        overall_score: Weighted overall SEO score (0-100).
...
class BlogGenerateRequest(BaseModel):
    """
    Request schema for generating an SEO-optimized blog post.

    Attributes:
        keywords: List of target keywords for the post.
        topic: Optional topic description (uses keywords if not provided).
    """

    keywords: list[str] = Field(..., min_length=1, description="Target keywords")
    topic: str | None = Field(None, description="Blog topic (optional, uses keywords if not provided)")


class BlogGenerateResponse(BaseModel):
    """
    Response schema for a generated blog post.
    ...
    Attributes:
        ...
        keywords: The target keywords used for generation.
    """
