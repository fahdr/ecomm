...
async def db_session():
    ...
    Creates a fresh engine per test to avoid connection conflicts from
    ``pg_terminate_backend`` calls during cleanup. Truncates all tables
    before yielding to ensure test isolation. Includes retry logic to
    ...
    **For Developers:**
        Each test gets its own engine and session. The cleanup phase
        terminates all other DB connections and truncates tables, then
        a separate session engine is created for the actual test work.
        Both engines are disposed at the end.
    """
    import asyncio
    ...
    # Phase 1: Cleanup (terminate stale connections + truncate).
    max_retries = 3
    ...
    # Phase 2: Create a fresh engine + session for the test.
    ...
    # Phase 3: Dispose the test engine to release connections.
