...
export default function StoreSettingsPage() {
  ...
  const [bridgeDeliveries, setBridgeDeliveries] = useState<BridgeDelivery[]>([]);
  ...
  useEffect(() => {
    ...
    /**
     * Fetch recent bridge activity for the store overview card.
     */
    async function fetchBridgeActivity() {
      setBridgeLoading(true);
      const res = await api.get<BridgeActivityResponse>(
        "/api/v1/bridge/activity?per_page=5"
      );
      if (res.data) {
        setBridgeDeliveries(res.data.items ?? []);
      }
    ...
    <PageTransition>
      <main className="mx-auto max-w-4xl p-6 space-y-6">
        ...
        <ServiceActivityCard
          ...
          maxItems={5}
          viewAllHref={storeId ? `/stores/${storeId}/services/activity` : undefined}
