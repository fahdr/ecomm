...
export default function ServiceDetailPage({
  ...
  // Service bridge activity state
  const [serviceDeliveries, setServiceDeliveries] = useState<BridgeDelivery[]>([]);
  ...
  useEffect(() => {
    ...
    /**
     * Fetch recent bridge activity for this specific service.
     */
    async function fetchServiceActivity() {
      setActivityLoading(true);
      const res = await api.get<BridgeDelivery[]>(
        `/api/v1/bridge/service/${serviceName}/activity?limit=10`
      );
      if (res.data) {
        setServiceDeliveries(res.data);
      }
      setActivityLoading(false);
    }
    fetchServiceActivity();
  }, [user, authLoading, fetchData, serviceName]);
      ...
      <main className="mx-auto max-w-4xl p-6 space-y-6">
        ...
        <FadeIn delay={0.25}>
          <ServiceActivityCard
            ...
            title={`Recent ${service.display_name} Activity`}
            showServiceName={false}
            maxItems={10}
            viewAllHref={`/stores/${storeId}/services/activity`}
