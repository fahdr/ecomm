...
async def get_usage(db: AsyncSession, user: User) -> dict:
    ...
    Queries actual generation job and image job counts for the current
    billing month. These metrics are displayed on the billing dashboard
    and enforced by the content generation service.
    ...
    Returns:
        Dict with plan, period dates, and metrics list containing
        generations_used and images_used with their limits.
    """
    from datetime import date

    from app.services.content_service import (
        count_monthly_generations,
        count_monthly_images,
    )
    ...
    generations_used = await count_monthly_generations(db, user.id)
    images_used = await count_monthly_images(db, user.id)
