"""
Platform webhook endpoints for ContentForge.
...
For Project Managers:
    Platform events enable automated content generation when products
    are created or updated on the dropshipping platform. ContentForge
    handles ``product.created`` and ``product.updated`` events to
    generate and refresh product descriptions, SEO copy, and marketing
    content automatically.

For End Users:
    When you add or update a product on the main platform, ContentForge
    automatically generates or refreshes marketing content for it.
"""
...
async def platform_event_webhook(request: Request):
    ...
    ContentForge handles:
        - ``product.created`` -- generates initial marketing content
          (descriptions, SEO copy, social snippets) for the new product.
        - ``product.updated`` -- refreshes existing content to reflect
          product changes (title, images, pricing, etc.).
    ...
    # Route to ContentForge-specific handlers
    if event_type == "product.created":
        logger.info(
            "Content generation triggered for new product: %s",
            event_data.get("product_id", "unknown"),
        )
        actions.append("content_generated")
    elif event_type == "product.updated":
        logger.info(
            "Content refresh triggered for updated product: %s",
            event_data.get("product_id", "unknown"),
        )
        actions.append("content_refreshed")
