"""
Tests for ContentForge AI suggestions service and API endpoints.

Covers AI-powered content enhancement and improvement suggestions,
...
For QA Engineers:
    ...
    - POST /api/v1/ai/enhance-content returns enhancements (authenticated).
    - Unauthenticated requests return 401.
    - Malformed LLM responses are handled gracefully.

For Project Managers:
    These tests ensure the AI content enhancement features in ContentForge
    work correctly and degrade gracefully.
"""
...
@pytest.mark.asyncio
async def test_enhance_content_unauthenticated(client: AsyncClient):
    """POST /api/v1/ai/enhance-content returns 401 without auth."""
    resp = await client.post("/api/v1/ai/enhance-content", json={"content_id": "test"})
    assert resp.status_code == 401
...
async def test_ai_suggestions_success(client: AsyncClient):
    ...
    mock_response = _mock_llm_response({
        "suggestions": [
            {"title": "Improve SEO titles", "description": "Add keywords to product titles", "priority": "high"},
            {"title": "Add bullet points", "description": "Use bullet points for features", "priority": "medium"},
        ]
    })
...
@pytest.mark.asyncio
async def test_enhance_content_success(client: AsyncClient):
    """POST /api/v1/ai/enhance-content returns enhancements when authenticated."""
    headers = await register_and_login(client)
    mock_response = _mock_llm_response({
        "enhancements": [
            {"category": "SEO", "current_issue": "Missing meta description", "suggestion": "Add meta description", "impact": "high"},
        ]
    })

    with patch("app.services.ai_suggestions_service.call_llm", new_callable=AsyncMock, return_value=mock_response):
        resp = await client.post(
            "/api/v1/ai/enhance-content",
            json={"content_id": "test-content-123"},
            headers=headers,
        )
    ...
    assert data["content_id"] == "test-content-123"
    assert "generated_at" in data
