"""
Platform webhook endpoint tests for ContentForge.
...
For QA Engineers:
    ...
    - Verifies correct event routing:
        product.created -> content_generated
        product.updated -> content_refreshed
...
async def test_platform_webhook_product_created(client: AsyncClient):
    """POST /webhooks/platform-events with product.created returns content_generated."""
    payload = json.dumps({
    ...
    assert "content_generated" in data["actions"]


@pytest.mark.asyncio
async def test_platform_webhook_product_updated(client: AsyncClient):
    """POST /webhooks/platform-events with product.updated returns content_refreshed."""
    payload = json.dumps({
    ...
    data = resp.json()
    assert data["status"] == "ok"
    assert "content_refreshed" in data["actions"]
