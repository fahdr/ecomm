"""
Test fixtures for the AdScale backend.

Uses shared test utilities from ecomm_core for database setup and auth helpers.

For Developers:
    Uses a dedicated PostgreSQL schema (``adscale_test``) to isolate from
...
def _make_engine():
    ...
    def set_search_path(dbapi_conn, connection_record):
        ...
        operations target the ``adscale_test`` schema instead of
        ``public``.
        """
...
async def create_tables():
    """
    Create the ``adscale_test`` schema and all tables once at the
    ...
    # Step 3: Create tables with a fresh engine (search_path = adscale_test only)
    last_error = None
    for attempt in range(3):
        engine = _make_engine()
        try:
...
async def truncate_tables():
    ...
    within the adscale_test schema. Retries up to 3 times on transient
