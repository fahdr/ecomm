...
(user or assistant) and content. The metadata JSON field stores additional
context like product references and clickable links.

For Developers:
    Messages are created in pairs: user sends a message, the AI generates
    a response. The `metadata` field can contain product suggestions,
...
For QA Engineers:
    ...
    Check that metadata contains valid product references when applicable.
...
class Message(Base):
    ...
    Attributes:
        ...
        metadata: Optional JSON with product references, links, etc.
